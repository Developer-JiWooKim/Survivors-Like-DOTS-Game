    ...
    public partial struct PlayerMoveSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
                     ...
                     in SystemAPI.Query<RefRW<LocalTransform>, RefRW<PlayerPosition>, RefRO<PlayerMovement>>())
            {
                ...
                if (hasGrid)
                {
                    // 반경은 플레이어 쿼드의 절반. 기획서에 플레이어 충돌 반경 규정이 없어 M3 임시값 0.5 를 쓴다.
                    float2 clamped = grid.ClampToBounds(transform.ValueRO.Position.xy, 0.5f);
                    transform.ValueRW.Position = new float3(clamped.x, clamped.y, transform.ValueRO.Position.z);
                }
